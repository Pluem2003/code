...
#include <BLEUtils.h>
#include <BLE2902.h>
#include "esp_sleep.h"  // ใช้สำหรับการประหยัดพลังงาน
...
const unsigned long readInterval = 10000; // กำหนดเวลาการอ่านข้อมูลทุก 10 วินาที
...
void loop() {
  ...
  if (deviceConnected) {
    ...
    if (millis() - lastReadTime > readInterval) {
      ...
      // เข้าสู่โหมด Light Sleep หลังจากส่งข้อมูล
      esp_sleep_enable_timer_wakeup(10 * 1000000); // ตั้งค่าให้เข้าสู่โหมด sleep ใน 10 วินาที
      esp_light_sleep_start(); // เข้าสู่โหมด Light Sleep
    }
  }
